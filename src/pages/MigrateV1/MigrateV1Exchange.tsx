    ...
    <>
      <AutoRow style={{ justifyContent: "flex-start", width: "fit-content" }}>
        ...
        <div style={{ marginLeft: ".75rem" }}>
          <TYPE.mediumHeader>
            ...
            {chainId && token.equals(ETH) ? "WETH" : token.symbol}/ETH
          </TYPE.mediumHeader>
        </div>
      </AutoRow>
      ...
      <RowBetween my="1rem">
        <Text fontSize={16} fontWeight={500}>
          Pooled {chainId && token.equals(ETH) ? "WETH" : token.symbol}:
        </Text>
        <RowFixed>
...
function V1PairMigration({
  ...
  const [v2PairState, v2Pair] = usePair(chainId ? ETH : undefined, token);
  const isFirstLiquidityProvider: boolean =
    v2PairState === PairState.NOT_EXISTS;

  const v2SpotPrice =
    chainId && v2Pair
      ? v2Pair.reserveOf(token).divide(v2Pair.reserveOf(ETH))
      : undefined;
  ...
  return (
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        ...
        ) : validatedAddress && chainId && token?.equals(ETH) ? (
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
