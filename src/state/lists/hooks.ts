...
import UNSUPPORTED_TOKEN_LIST from "../../constants/tokenLists/PancakeSwapDefaultList.tokenlist.json";
...
export function useActiveListUrls(): string[] | undefined {
  ...
  )?.filter((url) => !DEFAULT_LIST_OF_LISTS.includes(url));
}
...
export function useInactiveListUrls(): string[] {
  ...
  return Object.keys(lists).filter(
    (url) =>
      !allActiveListUrls?.includes(url) && !DEFAULT_LIST_OF_LISTS.includes(url)
  );
}
...
export function useCombinedActiveList(): TokenAddressMap {
  ...
  const defaultTokenMap = listToTokenMap(UNSUPPORTED_TOKEN_LIST);
...
export function useDefaultTokenList(): TokenAddressMap {
  return listToTokenMap(UNSUPPORTED_TOKEN_LIST);
}
